...
const Nurses = () => {
  ...
  const nurses = useSelector((state) => state.staff.nurses);
  ...
  // Only show essential fields for admin
  const allFields = ["id", "name", "department", "shift", "email", "phone"];
  const tableFields = ["id", "name", "department", "shift", "email", "phone"];
  ...
  useEffect(() => {
    ...
    const filtered = nurses.filter(
      (n) =>
        ...
        n.shift.toLowerCase().includes(query) ||
  ...
  const handleSave = () => {
    if (dialogMode === "edit") {
      dispatch(updateStaffRequest({ role: "nurses", staff: formData }));
    } else if (dialogMode === "add") {
      const newNurse = { ...formData, id: Date.now().toString() };
      dispatch(addStaffRequest({ role: "nurses", staff: newNurse }));
    }
    setOpenDialog(false);
  };
  ...
  const handleDelete = (id) => {
    ...
    setDeleteMessage("Nurse deleted successfully!");
    ...
    <Box className="container my-4">
      <Card className="shadow-sm">
        <CardContent>
          ...
          <div className="d-flex flex-wrap align-items-center mb-3 gap-2">
            <TextField
              placeholder="Search by name, department, shift, email, or phone..."
              value={searchQuery}
      ...
      >
        <DialogTitle sx={{ fontWeight: 'bold', fontSize: '1.5rem', color: '#1976d2' }}>
          {dialogMode === "view" ? "View Nurse" : dialogMode === "edit" ? "Edit Nurse" : "Add Nurse"}
        </DialogTitle>
