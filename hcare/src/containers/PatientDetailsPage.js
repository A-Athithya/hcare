...
export default function PatientDetailsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [p, apptsAll] = await Promise.all([
        getData(`/patients/${id}`),
        getData("/appointments"),
      ]);

      const pObj = p || null;
      const appts = Array.isArray(apptsAll) ? apptsAll.filter((a) => String(a.patientId) === String(id)) : [];
  ...
  if (!patient)
    return <div style={{ padding: 24, textAlign: "center" }}>Patient not found or access denied.</div>;

  const appointmentsColumns = [
    { title: "Date", dataIndex: "appointmentDate", key: "appointmentDate", render: (d) => dayjs(d).format("DD MMM YYYY") },
        ...
        <Col xs={24}>
          <Card
            ...
            extra={
              <div style={{ display: "flex", gap: 8 }}>
                <Button onClick={() => navigate(`/patient/edit/${id}`)}>Edit</Button>
                <Button type="primary" onClick={() => navigate(`/appointments?patientId=${id}`)}>
                  Book Appointment
                </Button>
          ...
          >
            <Descriptions bordered column={1} size="middle">
              ...
              <Descriptions.Item label="Registered Date">{patient.registeredDate ? dayjs(patient.registeredDate).format("DD MMM YYYY") : "—"}</Descriptions.Item>
              ...
              <Descriptions.Item label="Emergency Contact">{patient.emergencyContact || "—"}</Descriptions.Item>
              <Descriptions.Item label="Status">{patient.status || "—"}</Descriptions.Item>
              {/* Show any extra fields present in the patient object */}
              {Object.keys(patient).map((k) => {
                if (["id","name","age","gender","contact","email","address","bloodGroup","registeredDate","medicalHistory","allergies","emergencyContact","status"].includes(k)) return null;
                return (
                  <Descriptions.Item key={k} label={k}>
                    {typeof patient[k] === "object" ? JSON.stringify(patient[k]) : String(patient[k])}
                  </Descriptions.Item>
                );
              })}
            </Descriptions>
